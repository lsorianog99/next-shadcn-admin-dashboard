...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({
      chatId,
      content,
      role, // We ignore role here as it's always assistant if sent from UI
      messageType = "text",
    }: {
      chatId: string;
      content: string;
...
export function useUpdateChatStatus() {
  ...
  return useMutation({
    mutationFn: async ({ chatId, status }: { chatId: string; status: "active" | "archived" | "closed" }) => {
      const { data, error } = await supabase.from("chats").update({ status }).eq("id", chatId).select().single();
