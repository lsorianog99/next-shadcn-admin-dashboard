 ...
 */
export async function sendMessageToN8N(params: { chatId: string; phone: string; message: string }) {
  try {
...
export async function createQuoteFromN8N(params: {
  ...
  try {
    ...
    // Calcular totales
    const subtotal = params.products.reduce((sum, p) => sum + p.quantity * p.unit_price, 0);
 ...
 */
export async function getProductsCatalog(filters?: { category?: string; searchTerm?: string; limit?: number }) {
  try {
    const supabase = await createClient();
